...
if not urls:
    ...
    for url in urls:
        ...
        try:
            ...
            modified_date_str = modified_date.strftime("%Y") if modified_date else "Not Found"
            print(f" The modified date in stream is {modified_date_str}")
            ...
            invalid_pdfs.append({"url": url, "last_modified":"Not Found"})
...
#             if response.status_code == 200:
#                 last_modified = response.headers.get("Last-Modified", "Not Found")
#                 if last_modified:
