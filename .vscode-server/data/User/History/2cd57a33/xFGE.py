...
def upload_files_to_drive(service, folder_id):
    """Upload all images in the current directory to the specified Google Drive folder and delete them locally."""
    upload_files_to_drive = {'.pdf'}
    uploaded_files = []
    
    for filename in os.listdir('./'):
        if os.path.isfile(filename) and os.path.splitext(filename)[1].lower() in upload_files_to_drive:
            file_metadata = {
                'name': filename,
                'parents': [folder_id]
            }
...
def main():
    ...
    upload_files_to_drive(service, folder_id)
