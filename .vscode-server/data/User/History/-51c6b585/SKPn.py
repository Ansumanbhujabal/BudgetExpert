...
def streamlit_ui():
    ...
    st.write("Upload documents, query them, and get relevant answers.")
    
    query_input = st.text_input("Enter your query:")
    top_k = st.number_input("Number of top results (k):", min_value=1, value=2)
    
    if st.button("Get Answer"):
        if query_input.strip():
            with st.spinner("Retrieving answer..."):
                try:
                    doc_search = retrieve_query(query_input, top_k)
                    answer = chain.run(input_documents=doc_search, question=query_input)
