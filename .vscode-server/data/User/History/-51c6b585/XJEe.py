...
#     # Assuming `prompt` is a string containing the user input
#     response = llm.invoke(messages=[{"role": "user", "content": prompt}])

#     return response["choices"][0]["text"].strip()

def get_intent(user_input):
    # Prepare the input prompt in the expected format
    prompt = f"User asked: {user_input}. What is the intent?"

    # Call the Groq model with the correct 'input' argument, and messages formatted correctly
    response = llm.invoke(input={"messages": [{"role": "user", "content": prompt}]})

    # Process the response to get the intent (adjust based on the actual response structure)
    intent = response.get("answer", "Unknown intent")  # Assuming 'answer' is the expected field
    return intent
